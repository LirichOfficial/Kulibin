...
async def get_word(Topic):
    ...
    data1['messages'] = [
        {
            "role": "system",
            "text": "Твоя цель сгенерировать 10 популярных и узнаваемых объектов/персонажей в единственном числе по заданной теме. Нужно вывести только названия объектов/персонажей через запятую без дополнительной информации и знаков. Ни в коем случае не повторяйся"
        },
        {
            "role": "user",
...
async def get_answer(word, question):
    ...
    data1['messages'] = [
        {
            "role": "system",
            "text": 'Сейчас тебе будет дан персонаж/объект и вопрос про этот объект/персонаж, на который необходимо дать ответ в таком формате: Сначала выведи Да/Нет/Не знаю в зависимости от того, какой ответ на вопрос, а затем на следующеей строчке выведи пояснение к своему ответу, опираясь на факты. Ответ не должен противоречить пояснению.' 
        },
        {
            "role": "user",
